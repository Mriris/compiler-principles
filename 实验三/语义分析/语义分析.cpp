...
int call_stat(tree* father) {   // < call _stat>::= call ID‘(’ ‘)’ ;
	...
	while (strcmp("number", type) == 0 || strcmp("ID", type) == 0) {   //call xxx(yyy,....)
		...
		int placeI=0;
		if (strcmp("ID", type) == 0) {
			flagI = lookup(value, &placeI);       //在读取之前要先检查一下该变量是否被定义
			if (flagI > 0)
				return flagI;
...
int statement(tree* father) {
	...
	else if (flag == 0 && strcmp("while", type) == 0)
		flag = while_stat(current);
	else if (flag == 0 && strcmp("for", type) == 0)
		flag = for_stat(current);
	else if (flag == 0 && strcmp("read", type) == 0)
		flag = read_stat(current);
	else if (flag == 0 && strcmp("write", type) == 0)
		flag = write_stat(current);

	else if (flag == 0 && strcmp("call", type) == 0)
		flag = call_stat(current);
	else if (flag == 0 && (strcmp("(", type) == 0 || strcmp(";", type) == 0 ||
		strcmp("number", type) == 0 || strcmp("ID", type) == 0))
		flag = expression_stat(current);
	else if (flag == 0 && strcmp("do", type) == 0)    //do_while语句
...
int fun_declaration(tree* father) {     // <fun_declaration>::= function ID’(‘ ‘ )’< function_body>
	...
	while (strcmp("int", type) == 0) {   //int xxx (,....)
		...
		if (strcmp("ID", type) != 0)
			return 6;
		addChar(current);
		flag = insertSymbolTable(variable, value);
		if (flag > 0)
			return flag;
		funStack.push(symbolTable[symbolTable.size()-1].address);
		symbolTable[fPos].paraNum++;
		//检查是否存在多个参数
		getNext();
		if (strcmp(",", type) == 0) {       //有逗号说明有多个参数
			addChar(current);
	...
	if (flag == 0) {
		...
		for (int i = 0; i < symbolTable.size(); i++) {
			if(symbolTable[i].kind==1)
				printf("%-15s%-11d%-15s%-7s\n", symbolTable[i].name, symbolTable[i].address,"变量", "N/A");
			else if (symbolTable[i].kind == 0)
				printf("%-15s%-11d%-15s%-7d\n", symbolTable[i].name, symbolTable[i].address, "函数", symbolTable[i].paraNum,"N/A");
		}
		printf("\n");
		printf("中间代码\n%-5s%-16s%-16s\n", "序号", "操作码", "操作数");
