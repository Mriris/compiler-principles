...
int parse() {
	...
	for (int i = 0; i < symbolTable.size(); i++) {
		if (symbolTable[i].kind == 1)
			...
			printf("%-15s%-11d%-15s%-10d%-15s%-10d\n", symbolTable[i].name, symbolTable[i].address, "函数", symbolTable[i].paraNum, symbolTable[i].range.c_str(), (int)symbolTable[i].isReturn);
	}
	printf("\n");
	if (flag == 0) {
		printf("语义分析成功\n");
