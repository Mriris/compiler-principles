...
//运算因子
int factor(tree* father) {     // < factor >::=‘(’ < additive_expr >‘)’|ID|number
	...
	if (strcmp("(", type) == 0) {
		...
		if (strcmp("ID", type) == 0 || strcmp("number", type) == 0) {
			add(current);
			getNext();
			return flag;
		}
...
int statement(tree* father) {
	...
	if (flag == 0 && (strcmp("(", type) == 0 || strcmp(";", type) == 0 || strcmp("number", type) == 0) || strcmp("ID", type) == 0)
		flag = expression_stat(current);
	if (flag == 0 && strcmp("do", type) == 0)    //do_while语句
		flag = do_while_stat(current);
	if (flag == 0 && strcmp("return", type) == 0)    //return语句
		flag = returnT(current);
	if (flag == 0 && strcmp("int", type) == 0)    //return语句
		flag = declaration_list(current);
	return flag;
}
//return语句
int returnT(tree* father) {        //<return>::=return number;
	tree* current = father->addSon("<return>");
	int flag = 0;
	add(current);
	//检查 ID
	getNext();
...
//程序
int program(tree* root) {     //<program> ::={fun_declaration }<main_declaration>
	int flag = 0;
	root->data = "<program>";
	//检查 全局变量,可能有也可能没有
...
int main() {
	...
	flag = TESTparse();
	getNext();
	flag = program(root);//递归下降语法分析
	//处理结果

	while (flag != 0 || !feof(inputFile)) {
		err[success2][0] = line;
		err[success2++][1] = flag;
		char countNn[10] = "\0";
		do {
